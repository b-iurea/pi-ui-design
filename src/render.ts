...
export function setEditInfo(e: typeof _editInfo) {
	_editInfo = e;
}
	...
	switch (type) {
			...
			return `<div style="display:inline-flex;border:1px solid var(--border);border-radius:${r};overflow:hidden">${(
				...
				.map(
					(i: string, idx: number, a: string[]) =>
						`<span style="padding:5px 12px;font-size:11px;${idx < a.length - 1 ? "border-right:1px solid var(--border)" : ""};${i.trim() === p.active ? "background:var(--accent);color:#fff" : ""}">${i.trim()}</span>`,
				)
				.join("")}</div>`;
		case "iconBtn":
		...
		case "select":
			return `<div><div style="font-size:10px;color:var(--text-muted);margin-bottom:3px">${esc(p.label || "")}</div><select class="cp-select" style="border-radius:${r}">${(
				...
				.join("")}</select></div>`;
		case "multiSelect":
			return `<div><div style="font-size:10px;color:var(--text-muted);margin-bottom:3px">${esc(p.label || "")}</div><select multiple class="cp-select" style="border-radius:${r};min-height:60px">${(
				p.options || ""
			)
				.split(",")
				.map(
					(o: string) =>
						`<option${(p.selected || "").includes(o.trim()) ? " selected" : ""}>${o.trim()}</option>`,
				)
				.join("")}</select></div>`;
		case "checkbox":
		...
		case "autocomplete":
			return `<div><div style="font-size:10px;color:var(--text-muted);margin-bottom:3px">${esc(p.label || "")}</div><div style="position:relative"><input class="cp-input" placeholder="Type to search..." style="border-radius:${r}"><div style="position:absolute;top:100%;left:0;right:0;background:var(--bg2);border:1px solid var(--border);border-radius:4px;margin-top:2px;max-height:80px;overflow:hidden">${(
				p.suggestions || ""
			)
				.split(",")
				.slice(0, 3)
				.map(
					(s: string) =>
						`<div style="padding:4px 8px;font-size:10px;border-bottom:1px solid var(--border)">${s.trim()}</div>`,
				)
				.join("")}</div></div></div>`;
		case "chipInput":
		...
		case "navbar":
			return `<nav class="cp-navbar" style="border-radius:${r}"><span class="brand" style="color:${t.primary}">${esc(p.brand)}</span>${(
				...
				.split(",")
				.map((i: string) => `<span class="nav-item">${i.trim()}</span>`)
				.join("")}</nav>`;
		case "tabs":
			return `<div class="cp-tabs">${(p.items || "")
				...
				.join("")}</div>`;
		case "verticalTabs":
			return `<div style="display:flex;gap:2px;flex-direction:column;padding:4px">${(
				p.items || ""
			)
				.split(",")
				.map(
					(i: string) =>
						`<span style="padding:5px 10px;font-size:11px;border-radius:4px;${i.trim() === p.active ? "background:var(--accent);color:#fff" : "background:var(--bg3)"}">${i.trim()}</span>`,
				)
				.join("")}</div>`;
		case "breadcrumb":
		...
		case "dropdown":
			return `<div class="cp-dropdown"><span class="cp-dropdown-trigger">${esc(p.trigger)} ⏷</span><div class="cp-dropdown-menu">${(
				...
				.split(",")
				.map((i: string) => `<div class="cp-dropdown-item">${i.trim()}</div>`)
				.join("")}</div></div>`;
		case "megaMenu":
			return `<div style="display:flex;gap:16px;padding:12px;background:var(--bg2);border:1px solid var(--border);border-radius:${r}">${(
				p.items || ""
			)
				.split(",")
				.map((i: string) => {
					const [hd, ...rest] = i.split("~");
					return `<div style="min-width:100px"><div style="font-weight:600;font-size:11px;margin-bottom:6px;color:var(--text)">${esc(hd || "")}</div>${rest.length ? rest.map((r: string) => `<div style="font-size:10px;color:var(--text-muted);padding:3px 0">${r.trim()}</div>`).join("") : ""}</div>`;
				})
				.join("")}</div>`;
		case "hamburger":
		...
		case "stepper":
			return `<div style="display:flex;align-items:center;gap:4px;padding:4px">${(
				p.steps || ""
			)
				.split(",")
				.map(
					(s: string, i: number) =>
						`<div style="display:flex;align-items:center;gap:2px"><span style="width:20px;height:20px;display:flex;align-items:center;justify-content:center;border-radius:50%;font-size:9px;font-weight:600;${i < (Number(p.current) || 1) ? "background:var(--accent);color:#fff" : "background:var(--bg3);color:var(--text-muted)"}">${i + 1}</span><span style="font-size:9px;color:var(--text-muted);${i < (Number(p.current) || 1) ? "color:var(--text)" : ""}">${s.trim()}</span>${i < (p.steps || "").split(",").length - 1 ? '<span style="color:var(--border)">─</span>' : ""}</div>`,
				)
				.join("")}</div>`;
		case "backToTop":
		...
		case "list":
			return `<ul class="cp-list">${(p.items || "")
				...
				.join("")}</ul>`;
		case "orderedList":
			return `<ol style="list-style:decimal;padding-left:18px;font-size:12px">${(
				p.items || ""
			)
				.split(",")
				.map((i: string) => `<li style="margin:2px 0">${i.trim()}</li>`)
				.join("")}</ol>`;
		case "table":
		...
		case "timeline":
			return `<div style="display:flex;flex-direction:column;gap:8px;padding:4px">${(
				...
				.split(",")
				.map((item: string) => {
					const [t, d, date] = item.split("~");
					return `<div style="display:flex;gap:8px"><div style="display:flex;flex-direction:column;align-items:center;width:12px"><div style="width:8px;height:8px;border-radius:50%;background:var(--accent);flex-shrink:0"></div><div style="width:1px;flex:1;background:var(--border);margin:2px 0"></div></div><div style="flex:1"><div style="font-weight:600;font-size:11px">${esc(t || "")}</div><div style="font-size:10px;color:var(--text-muted)">${esc(d || "")}</div><div style="font-size:9px;color:var(--text-muted);opacity:.6">${esc(date || "")}</div></div></div>`;
				})
				.join("")}</div>`;
		case "treeView":
			return `<div style="padding:4px;font-size:11px">${(p.items || "")
				.split("|")
				.map(
					(level: string, i: number) =>
						`<div style="padding-left:${i * 16}px;display:flex;align-items:center;gap:4px;padding-top:2px;padding-bottom:2px">${level
							.split(",")
							.map(
								(item: string) =>
									`<span style="color:var(--text-muted)">${item.trim()}</span>`,
							)
							.join(", ")}</div>`,
				)
				.join("")}</div>`;
...
export function renderResizeHandles(): void {
	...
	handles.innerHTML = "n,s,e,w,ne,nw,se,sw"
		.split(",")
		.map((d) => `<div class="resize-handle ${d}" data-direction="${d}"></div>`)
		.join("");
}
...
function renderCanvas(): void {
	...
	if (!list) return;
	list.innerHTML =
		totalComponents(S.tree) === 0 ? "" : renderTreeNode(S.tree, 0);
	const empty = document.getElementById("canvas-empty");
	if (empty)
		empty.style.display = totalComponents(S.tree) === 0 ? "block" : "none";
}
...
function renderInspector(): void {
	...
	const masterHTML = c._masterId
		? `<div class="ins-section"><div class="ins-header">Master <span style="font-size:9px;color:var(--accent)">◆ ${esc(c._masterName || "")}</span></div><div class="ins-body"><button class="btn-sm" onclick="window.UI.detachInstance('${c.id}')">Detach instance</button></div></div>`
		: "";
	const textHTML =
		p.text !== undefined
			? `<div class="ins-row"><label>Text</label><input type="text" value="${esc(p.text)}" onchange="window.UI.updateProp('${c.id}','text',this.value)"></div>`
			: "";
	const labelHTML =
		p.label !== undefined
			? `<div class="ins-row"><label>Label</label><input type="text" value="${esc(p.label)}" onchange="window.UI.updateProp('${c.id}','label',this.value)"></div>`
			: "";
	const placeholderHTML =
		p.placeholder !== undefined
			? `<div class="ins-row"><label>Placeholder</label><input type="text" value="${esc(p.placeholder)}" onchange="window.UI.updateProp('${c.id}','placeholder',this.value)"></div>`
			: "";
	const colorHTML =
	...
	panel.innerHTML =
		...
		sizingHTML +
		layoutHTML +
		`<div class="ins-section"><div class="ins-header">${def.cat} / ${def.label}</div><div class="ins-body">${textHTML}${labelHTML}${placeholderHTML}${colorHTML}${Object.entries(
			p,
		)
			.filter(
				([k]) =>
			...
			.map(([key, val]) => {
				...
				if (["checked", "on", "featured", "vertical", "multiple"].includes(key))
					return `<div class="ins-row"><label>${lbl}</label><input type="checkbox" ${val ? "checked" : ""} onchange="window.UI.updateProp('${c.id}','${key}',this.checked)"></div>`;
				if (key === "level")
					return `<div class="ins-row"><label>${lbl}</label><select onchange="window.UI.updateProp('${c.id}','${key}',this.value)">${["h1", "h2", "h3", "h4", "h5", "h6"].map((l) => `<option ${l === val ? "selected" : ""}>${l}</option>`).join("")}</select></div>`;
				if (key === "variant")
					return `<div class="ins-row"><label>${lbl}</label><select onchange="window.UI.updateProp('${c.id}','${key}',this.value)">${["info", "warn", "error", "primary", "secondary", "success"].map((v) => `<option ${v === val ? "selected" : ""}>${v}</option>`).join("")}</select></div>`;
				if (key === "direction")
					return `<div class="ins-row"><label>${lbl}</label><select onchange="window.UI.updateProp('${c.id}','${key}',this.value)"><option ${val === "vertical" ? "selected" : ""}>vertical</option><option ${val === "horizontal" ? "selected" : ""}>horizontal</option></select></div>`;
				if (key === "active")
					return `<div class="ins-row"><label>${lbl}</label><select onchange="window.UI.updateProp('${c.id}','${key}',this.value)">${(
						p.options ||
						p.items ||
						""
					)
				...
				if (
					...
					return `<div class="ins-row"><label>${lbl}</label><input type="number" value="${val}" onchange="window.UI.updateProp('${c.id}','${key}',parseInt(this.value)||0)"></div>`;
