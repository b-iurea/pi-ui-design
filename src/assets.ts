/* ═══ Asset management — built-in icons + uploaded images ═══ */

import type { BuiltinIcon } from "./types";
import { S, pushHistory, scheduleSave } from "./state";
import { nextId, findNode, isContainer } from "./utils";
...
export function uploadAsset(): void {
	...
	input.onchange = (e: Event) => {
		...
		reader.onload = () => {
			S.assets.push({ id: "a" + Date.now(), name: file.name, dataUrl: reader.result as string, type: "image" });
			render();
			toast("Asset added");
		};
...
export function addAssetToCanvas(assetId: string): void {
	const asset = S.assets.find((a) => a.id === assetId);
	if (!asset) return;
	const id = nextId();
	const node: any = { id, type: "image", props: { alt: asset.name, src: asset.dataUrl, width: "200px", height: "160px" }, children: [] };
...
export function addIconToCanvas(iconName: string): void {
	const icon = BUILTIN_ICONS.find((i) => i.name === iconName);
	if (!icon) return;
	const id = nextId();
	const node: any = { id, type: "icon", props: { symbol: icon.svg, size: "24px" }, children: [] };
...
export function renderAssets(): void {
	...
	// renderPalette is called by the palette module on init
	const catContainer = document.getElementById("palette-categories");
	if (!catContainer) return;
	const uploads = S.assets.length ? S.assets.map((a) =>
		`<div class="asset-item" draggable="true" data-asset-id="${a.id}" onclick="window.UI.addAssetToCanvas('${a.id}')">
	...
	const iconRows = BUILTIN_ICONS.map((icon) =>
		`<div class="asset-item" draggable="true" data-icon-name="${icon.name}" onclick="window.UI.addIconToCanvas('${icon.name}')">
