...
export function renderPalette(): void {
	...
	if (!container) return;
	const q = ((document.getElementById("palette-search") as HTMLInputElement)?.value || "").toLowerCase();
	const cats: Record<string, { type: string; label: string; icon: string; props: any }[]> = {};
