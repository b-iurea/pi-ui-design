...
let _isPanning = false,
	_panStart: { x: number; y: number } | null = null;
let _isMarquee = false,
	_marqueeStart: { x: number; y: number } | null = null;
let _resizeInfo: {
	id: string;
	dir: string;
	startX: number;
...
export function setupCanvasEvents(): void {
	...
	container.addEventListener("mousedown", (e: MouseEvent) => {
		...
		if (e.button === 0 && S.tool === "select") {
			...
			if (handle) {
				...
				_resizeInfo = {
					id,
					dir: (handle as HTMLElement).dataset.direction!,
					startX: e.clientX,
					startY: e.clientY,
					startW: rect.width,
					startH: rect.height,
	...
	container.addEventListener("click", (e: MouseEvent) => {
		if (
			!(e.target as HTMLElement).closest(".canvas-component") &&
...
export function setDragInfo(d: any) {
	_dragInfo = d;
}
