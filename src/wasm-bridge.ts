...
export async function loadEngine(): Promise<EngineModule> {
	...
	await new Promise<void>((resolve, reject) => {
		...
		script.onload = () => {
			...
			const check = () => {
				if (
					...
					typeof (window as any).Module._addNode === "function"
				) {
					resolve();
		...
		script.onerror = () =>
			reject(new Error("Failed to load engine.wasm glue code"));
	...
	_engine = {
		...
		moveNode: (id: number, newParentIdx: number, index: number) =>
			mod.moveNode(id, newParentIdx, index),
		updateProp: (id: number, key: string, value: string) =>
			mod.updateProp(id, key, value),
		setBounds: (id: number, x: number, y: number, w: number, h: number) =>
			mod.setBounds(id, x, y, w, h),
		nodeCount: () => mod.nodeCount(),
		getCommandBuffer: () => mod.getCommandBuffer(),
		commandCount: () => mod.commandCount(),
		serialize: () => mod.serialize(),
		deserialize: (buf: { ptr: number; byteLength: number }) =>
			mod.deserialize(buf),
		reset: () => mod.reset(),
	};
